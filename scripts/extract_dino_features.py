...
class ViTExtractor:
    ...
    def preprocess(self, image_path: Union[str, Path],
        ...
        """
        pil_image = image_from_stream(image_path).convert('RGB')
...
def _get_opts() -> argparse.Namespace:
    ...
    parser.add_argument('--load_size', default=350, type=int, help='load size of the input image.')
...
def main(hparams: argparse.Namespace) -> None:
    ...
    for i in tqdm(np.arange(rank, len(frames), world_size)):
        tmp_feature_path = '{}.pt'.format(frames[i]['feature_path'])
        fs = get_filesystem(tmp_feature_path)

        if (fs is None and Path(tmp_feature_path).exists()) or (fs is not None and fs.exists(tmp_feature_path)):
            try:
                torch.load(buffer_from_stream(tmp_feature_path), map_location='cpu')
                continue
            except:
                traceback.print_exc()

        if fs is None:
            parent = Path(tmp_feature_path).parent
        ...
        with open(tmp_feature_path, 'wb') as f:
